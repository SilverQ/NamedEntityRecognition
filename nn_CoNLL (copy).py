...
from keras.layers import TimeDistributed, Conv1D, Dense, Embedding, Input, Dropout, LSTM
from keras.layers import Bidirectional, MaxPooling1D, Flatten, concatenate
from prepro import readfile, createBatches, createMatrices, iterate_minibatches, addCharInformation, padding
from keras.utils import plot_model
from keras.initializers import RandomUniform, glorot_normal, glorot_uniform, lecun_uniform, he_normal, lecun_normal, he_uniform
...
class CNN_BLSTM(object):
    ...
    def buildModel(self):
        ...
        # CNN
        conv1d_out = TimeDistributed(Conv1D(kernel_size=self.conv_size, filters=30,
                                            padding='same', activation='tanh', strides=1), name="Convolution")(dropout)
        ...
        words_output = Bidirectional(LSTM(self.lstm_state_size, return_sequences=True,
                                          dropout=self.dropout,  # on input to each LSTM block
                                          recurrent_dropout=self.dropout_recurrent),  # on recurrent input signal
                                     name="words_BLSTM_1")(words)
        words = Bidirectional(LSTM(self.lstm_state_size, return_sequences=True,
                                   dropout=self.dropout,  # on input to each LSTM block
                                   recurrent_dropout=self.dropout_recurrent
                                   ),  # on recurrent input signal
                              name="words_BLSTM_2")(words_output)
        # casing_output = Bidirectional(LSTM(self.lstm_state_size, return_sequences=True,
        #                                    dropout=self.dropout,  # on input to each LSTM block
        #                                    recurrent_dropout=self.dropout_recurrent),  # on recurrent input signal
        #                               name="casing_BLSTM_1")(casing)
        # casing_output = Bidirectional(LSTM(self.lstm_state_size, return_sequences=True,
        #                                    dropout=self.dropout,  # on input to each LSTM block
        #                                    recurrent_dropout=self.dropout_recurrent),  # on recurrent input signal
        #                               name="casing_BLSTM_2")(casing_output)
        char_output = Bidirectional(LSTM(self.lstm_state_size, return_sequences=True,
                                         dropout=self.dropout,  # on input to each LSTM block
                                         recurrent_dropout=self.dropout_recurrent
                                         ),  # on recurrent input signal
                                    name="char_BLSTM_1")(char)
        char = Bidirectional(LSTM(self.lstm_state_size, return_sequences=True,
                                  dropout=self.dropout,  # on input to each LSTM block
                                  recurrent_dropout=self.dropout_recurrent
                                  ),  # on recurrent input signal
                             name="char_BLSTM_2")(char_output)

        output = concatenate([words, casing, char])
        output = Dense(len(self.label2Idx), activation='relu', name="Dense_layer")(output)
...
EPOCHS = 60               # paper: 80,
# original 코드, tensorflow-cpu : 7분 36초 소요(Epoch = 10)
# blstm을 concat 앞으로 변경, cpu : 15분 50초 소요(Epoch = 10)
